...
static void av_start(int32_t call_index, void *arg)
{
    ...
    if(video && video_width) {
        settings.max_video_width = video_width;
        settings.max_video_height = video_height;
    }

    if(toxav_prepare_transmission(arg, call_index, &settings, video) == 0) {
        if(video) {
            postmessage(FRIEND_CALL_START_VIDEO, fid, call_index, (void*)(settings.max_video_width | (size_t)settings.max_video_height << 16));
        } else {
